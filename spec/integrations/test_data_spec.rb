  ...
  module Integrations
    describe 'TestData' do
      it 'is a valid datasource' do
        ...
        expect(config.feature_store.all(LaunchDarkly::Impl::DataStore::FEATURES)).to eql({})
        client.close
      end
      ...
      it 'initializes the feature store with existing flags' do
        ...
        config = Config.new(send_events: false, data_source: td)
        client = LDClient.new('sdkKey', config)
        expect(config.feature_store.get(LaunchDarkly::Impl::DataStore::FEATURES, 'flag').data).to eql({
          key: 'flag',
      ...
      it 'updates the feature store with new flags' do
        ...
        expect(config2.feature_store.get(LaunchDarkly::Impl::DataStore::FEATURES, 'flag').data).to eql({
          key: 'flag',
          variations: [true, false],
          fallthrough: { variation: 0 },
          offVariation: 1,
      ...
      it 'can include preconfigured items' do
        ...
        expect(config.feature_store.get(LaunchDarkly::Impl::DataStore::FEATURES, 'my-flag').data).to eql({
          key: 'my-flag', version: 1000, on: true
        })
        expect(config.feature_store.get(LaunchDarkly::Impl::DataStore::SEGMENTS, 'my-segment').data).to eql({
