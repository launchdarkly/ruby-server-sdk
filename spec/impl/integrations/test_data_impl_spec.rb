      ...
      describe 'TestData' do
        ...
        describe 'FlagBuilder' do
          ...
          it 'defaults to targeting on and sets the flag key' do
            f = TestDataImpl::FlagBuilder.new('flag').build(1)
          ...
          it 'can set targeting off' do
            f = TestDataImpl::FlagBuilder.new('flag').on(false).build(1)
            expect(f[:on]).to eq(false)
          end

          it 'can set fallthrough variation' do
            f = TestDataImpl::FlagBuilder.new('flag').fallthrough_variation(0).build(1)
            expect(f[:fallthrough][:variation]).to eq(0)
          end

          it 'can set variation for when targeting is off' do
            f = TestDataImpl::FlagBuilder.new('flag').off_variation(0).build(1)
            expect(f[:off_variation]).to eq(0)
          end

          it 'can set a list of variations' do
            f = TestDataImpl::FlagBuilder.new('flag').variations(true, false).build(1)
            expect(f[:variations]).to eq([true, false])
          end

          it 'has the boolean_flag shortcut method' do
            f = TestDataImpl::FlagBuilder.new('flag').boolean_flag.build(1)
            expect(f[:variations]).to eq([true, false])
            expect(f[:fallthrough][:variation]).to eq(0)
            expect(f[:off_variation]).to eq(1)
          end
          ...
          it 'can handle boolean or index variation' do
            f = TestDataImpl::FlagBuilder.new('flag').off_variation(true).build(1)
            expect(f[:variations]).to eq([true, false])
            expect(f[:off_variation]).to eq(0)

            f2 = TestDataImpl::FlagBuilder.new('flag').fallthrough_variation(true).build(1)
            expect(f2[:variations]).to eq([true, false])
            expect(f2[:off_variation]).to eq(1)
          end

          it 'can set variation for all users' do
            f = TestDataImpl::FlagBuilder.new('flag').variation_for_all_users(true).build(1)
          ...
          it 'clears existing rules when setting variation for all users' do
            f = TestDataImpl::FlagBuilder.new('flag')
                            .if_match('name', 'ben')
                            .then_return(false)
          ...
          it 'can make an immutable copy of its self' do
            fb = TestDataImpl::FlagBuilder.new('flag').variation_for_all_users(true)
