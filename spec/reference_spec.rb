...
module LaunchDarkly
  describe Reference do
    ...
    it("can handle equality comparisons") do
      [
        [subject.create("name"), subject.create("name"), true],
        [subject.create("name"), subject.create("/name"), true],
        [subject.create("/first/name"), subject.create("/first/name"), true],
        [subject.create_literal("/name"), subject.create_literal("/name"), true],
        [subject.create_literal("/name"), subject.create("/~1name"), true],
        [subject.create_literal("~name"), subject.create("/~0name"), true],

        [subject.create("different"), subject.create("values"), false],
        [subject.create("name/"), subject.create("/name"), false],
        [subject.create("/first/name"), subject.create("/first//name"), false],

      ].each do |(lhs, rhs, expectation)|
        expect(lhs == rhs).to be expectation
      end
    end
  end
end
